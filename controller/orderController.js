...
const checkPaymentStatusPeriodically = (sessionId, metadata, io) => {
      ...
      try {
        ...
        if (session.payment_status === 'paid') {
          ...
          const newOrder = new Order({
            ...
            CGST,
            grandTotal,
            extraCharges, // Save extraCharges in the database
            status: 'Paid',
          ...
          // Emit payment status to the frontend
          ...
          try {
            ...
            const stopIds = response.data.data.stops.map((stop, index) => ({
              stopId: stop.stopId,
              name: index === 0 ? 'Michal' : 'Katrina', // Example names, replace as needed
              phone: index === 0 ? '+85238485765' : '+85238485696'
            }));
            ...
            const placeOrderBody = {
              "data": {
                ...
                "sender": {
                  "stopId": stopIds[0].stopId,
                  "name": stopIds[0].name,
                  "phone": stopIds[0].phone
                },
                "recipients": [
                  {
                    "stopId": stopIds[1].stopId,
                    "name": stopIds[1].name,
                    "phone": stopIds[1].phone,
            ...
            try {
              ...
              // Emit order details to the frontend
              io.to(userId).emit('order_details', { orderDetails: newOrder });
...
const calculateTotalAndInitiatePayment = async (req, res) => {
  ...
  try {
    ...
    try {
      ...
      const result = await checkPaymentStatusPeriodically(session.id, session.metadata, io);
      console.log(result);
    } catch (error) {
      console.error(error);
    }
...
module.exports = { stripe, calculateTotalAndInitiatePayment, checkPaymentStatusPeriodically };
...
//           // Call Lalamove Quotation API
//           const lalamoveApiKey = 'pk_test_09c0de17ca81b422b1d0df74f4c89d12';
//           const lalamoveSecret = 'sk_test_Q79qoiEpEHTQE0ajj1/1/i+YVKel84qyTyRPmqfTFZ3DUq+H3cMjzSzAxAaJ36aj'; // Replace with your Lalamove secret
//           const market = 'HK';
//           const timestamp = new Date().getTime();
...
//               // Emit order details to the frontend
//               io.emit('order_details', { orderDetails: newOrder });
